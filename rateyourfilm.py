...
SECRET_KEY = "ed688834b5e16f5af8e084cff73d55e64cbc4aa4573e5e7fae22d2acb27ecfc5"
ALGORITHM = "HS256"
...
@app.post("/movies")
def post_movie(movie:Movie, current_user_iid: int = Depends(get_current_user)):
    conn = mdb_pool.get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
...
def get_movie(title: str):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT id, title, year, details FROM movies WHERE title LIKE %s",
                       (f"%{title}%"),
                       )
        rows = cursor.fetchall()
        return [{"id":[0], "title":r[1], "year released":r[2], "More info":r[3]}
                for r in rows]
...
def rate_movie(data: Rating,current_user_id:int = Depends(get_current_user) ):
    if data.rating < 1 or data.rating > 10:
        raise HTTPException(status_code=400, detail="rating must be between 1-10")
    conn = mdb_pool.get_connection()
    cursor = conn.cursor()
    try:#check if user already have rated this movie
        cursor.execute("INSERT INTO rating (user_id, movie_id, rating)",
